...
class HomeController extends Controller
{
    ...
    {
        $product = Product::with('game', 'category')->where('slug', $slug)->first();
        if (!$product) {
            return abort(404);
        }
        ...
        if ($payment->provider == 'tokopay') {
            ...
            if (isset($invoice['status'])) {
                ...
                History::create([
                    ...
                    'product' => ['game' => $game->name, 'product_id' => $denom->product->id, 'name_provider' => $denom->product->name, 'denom_id' => $denom->id, 'denom' => $denom->name],
